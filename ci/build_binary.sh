...
build_lambda_cgo() {
	...
	local filename="${TRAVIS_BUILD_DIR}/releases/tegola_${GOOS}_${GOARCH}"
	xgo -go 1.12.x --targets="${GOOS}/${GOARCH}" -ldflags "${LDFLAGS}" -dest "${TRAVIS_BUILD_DIR}/releases" github.com/go-spatial/tegola/cmd/tegola_lambda

	cd $(dirname $filename)
	zip -9 -D tegola_lambda_cgo.zip tegola_lambda
	rm -f tegola_lambda
}
