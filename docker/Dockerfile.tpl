...
RUN mkdir -p /opt/tegola_src/src/github.com/terranodo/tegola
COPY . /opt/tegola_src/src/github.com/terranodo/tegola

# Build binary
WORKDIR /opt/tegola_src/src/github.com/terranodo/tegola/cmd/tegola
ENV GOPATH=/opt/tegola_src
RUN go build -v {{.flags}} -gcflags "-N -l" -o /opt/tegola github.com/terranodo/tegola/cmd/tegola
RUN chmod a+x /opt/tegola

# --- Create minimal deployment image, just alpine & the binary
FROM alpine:3.7
LABEL maintainer="{{.maintainer}}"
LABEL io.terranodo.version="{{.version}}"
